    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded shadow w-full max-w-md max-h-[90vh] overflow-auto p-6">
        ...
        <h2 class="text-xl font-bold mb-4">
          {editingIssueId ? "Edit" : "Create"} Issue
        </h2>
        ...
        {#if newIssue.attachment}
          <div class="mb-4">
            ...
            {:else}
              <img
                src={`${BASE_URL}${newIssue.attachment.url}`}
                alt={newIssue.attachment.filename}
                class="w-full max-h-64 object-contain border rounded mb-2"
              />
            {/if}

            <a
              href={`${BASE_URL}${newIssue.attachment.url}`}
              target="_blank"
              download={newIssue.attachment.filename}
